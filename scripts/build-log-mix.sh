...
#                 gtmstack.ai" + music)
#   4s  -> end  : narration at full volume, music ducks under to ~0.18
#                 (about -15 dB) so the cloned voice is the prominent sound
...
# Filter graph:
#   [0:a] ducked intro/music: full volume for 4s, then 0.18 (-15 dB)
#   [1:a] narration delayed 4s on both stereo channels
#   amix the two; cap to TOTAL_DUR; fade out final 2s
ffmpeg -y -hide_banner -loglevel warning \
  -i "$INTRO" \
  -i "$VOICE" \
  -filter_complex "
    [0:a]volume=volume='if(lt(t,4),1.0,0.18)':eval=frame[ducked];
    [1:a]adelay=4000|4000[delayed];
